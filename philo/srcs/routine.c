...
/*   Updated: 2022/08/08 12:31:03 by dantremb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_eating(void *arg)
{
	...
	while (1)
	{
		...
		ft_sleep(philo->last_meal + philo->table->eat);
		...
		if (philo->eated_meal == philo->table->meal_count)
			break ;
		ft_sleep(philo->last_meal + philo->table->sleep + philo->table->eat);
		printf("%lu %d is thinking\n", ft_get_ms(philo), philo->name);
		ft_sleep(((philo->last_meal + philo->table->die) / 100) * 90);
	}
